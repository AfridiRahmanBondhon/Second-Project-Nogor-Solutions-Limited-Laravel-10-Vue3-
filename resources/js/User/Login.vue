        ...
        <div class="card">
            ...
            <div class="card-body">
                <form id="registration" enctype="multipart/form-data" >
                    ...
                    <div class="mb-3">
                        <button class="btn btn-primary col-md-4" @click.prevent="checkdata()" type="button">Log In</button>
                    </div>
                </form>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            checkdata(){
                  axios.post(`/api/userAuthentication/match`,this.model.user)
                      .then(response=>{
                          if (response.data.status==200){
                                localStorage.setItem("lastname","ARB");
                                //this.$router.push('');
                              window.location.href='/admin/editUser';
                              toaster.success(`Login Successful`,{position:"top",diration:5000});
                          }else {
                              toaster.success(`No such user. Please Register!!`,{position:"top",diration:5000});
                          }
                      })
